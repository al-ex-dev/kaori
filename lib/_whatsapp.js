		...
		getMessageBody: {
			value(type, msg) {
				const body = {
					...
					interactiveResponseMessage: () => JSON.parse(msg.nativeFlowResponseMessage.buttonParamsJson).id,
					text: () => msg.text
				}
			...
			value(stream) {
				return new Promise((resolve, reject) => {
					...
					ffmpeg(stream)
						...
						.outputOptions('-avoid_negative_ts', 'make_zero', '-ac', '1')
						.audioBitrate(128)
						.on('error', (err) => {
							reject(err)
						})
